...
def run(dataset,neurons=160,activation='tanh',init_mode = 'uniform',optimizer='Nadam',epochs=20000):
    ...
    colors = iter(cm.gist_rainbow(numpy.linspace(0, 1, len(init_mode_list))))
    ...
    for init_mode in init_mode_list:
        ...
        color=next(colors)
        plt.subplot(211)
        plt.plot(history.history['acc'], label="Train "+init_mode, color=color, linestyle = 'solid')
        plt.plot(history.history['val_acc'], label="Test "+init_mode, color=color, linestyle = 'dashed')
        
        plt.subplot(212)
        plt.plot(history.history['loss'], label="Train "+init_mode, color=color, linestyle = 'solid')
        plt.plot(history.history['val_loss'], label="Test "+init_mode, color=color, linestyle = 'dashed')
    ...
    plt.savefig("InitMode-4layers-%dneurons-%s-%depochs.png" %(neurons,activation,epochs), dpi=300)
