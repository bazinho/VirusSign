...
def run(dataset,neurons=160,activation='tanh',init_mode = 'uniform',optimizer='Nadam',epochs=20000):
    try:
    ...
    colors = iter(cm.gist_rainbow(numpy.linspace(0, 1, len(activation_list))))
    ...
    for activation in activation_list:
        ...
        color=next(colors)
        plt.subplot(211)
        plt.plot(history.history['acc'], label="Train "+activation, color=color, linestyle = 'solid')
        plt.plot(history.history['val_acc'], label="Test "+activation, color=color, linestyle = 'dashed')
        
        plt.subplot(212)
        plt.plot(history.history['loss'], label="Train "+activation, color=color, linestyle = 'solid')
        plt.plot(history.history['val_loss'], label="Test "+activation, color=color, linestyle = 'dashed')
    ...
    plt.savefig("Activations-4layers-%dneurons-%s-%depochs.png" %(neurons,optimizer,epochs), dpi=300)
