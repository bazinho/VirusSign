...
def run(dataset,hlayers=4,neurons=160,activation='tanh',init_mode = 'uniform',optimizer='Nadam',epochs=20000):
    try:
    ...
    hlayers_list = range(1,hlayers+1)
    colors = iter(cm.gist_rainbow(numpy.linspace(0, 1, len(hlayers_list))))
    ...
    for layers in hlayers_list:
        ...
        for l in range(2,layers):
            #n = n/2
            model.add(Dense(n, init=init_mode, activation=activation))
            #model.add(Dropout(0.5))
    ...
    plt.savefig("Layers-%dneurons-%s-%depochs.png" %(neurons,activation,epochs), dpi=300)
