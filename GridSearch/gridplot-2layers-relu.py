...
def run(dataset):
    ...
    for neurons in neurons_list:
        print("\nNeurons: Layer 1: %d - Layer 2: %d" %(neurons,neurons/2))
        start = time.time()
        model = Sequential()
    ...
    plt.savefig("Grid-2Layers-%depochs.png" %(epochs), dpi=100)
    # plt.legend(optimizer_list, bbox_to_anchor=(0, 0, 1, 1), bbox_transform=plt.gcf().transFigure)
    # plt.show()
  
if __name__ == '__main__':
    parse = argparse.ArgumentParser('gridplot-2layers.py', description='Multi Layer Perceptron classifier.')
