...
def run(dataset):
    ...
    for optimizer in optimizer_list:
        ...
        plt.subplot(211)
        plt.plot(history.history['acc'], label=optimizer, linewidth=2.0)
        plt.subplot(212)
        plt.plot(history.history['loss'], label=optimizer, linewidth=2.0)
    tend = time.time()
    ...
    plt.savefig("Grid-Optimizer-%depochs.png" %(epochs), dpi=100)
    # plt.legend(optimizer_list, bbox_to_anchor=(0, 0, 1, 1), bbox_transform=plt.gcf().transFigure)
    # plt.show()
  
if __name__ == '__main__':
    parse = argparse.ArgumentParser('gridplot-optimization.py', description='Multi Layer Perceptron classifier.')
