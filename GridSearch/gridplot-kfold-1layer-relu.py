...
def run(dataset):
    ...
    epochs=10 # about 60 s (1 min) for each 1000 epochs
    optimizer = 'Adam'
    init_mode = 'uniform'
    activation = 'relu'
    neurons = 160
    ...
    for train, test in kfold.split(X, y):
        ...
        color=next(colors)
        plt.subplot(221)
        plt.plot(trainacc, label="Train "+str(fold), color=color, linewidth=2.0, linestyle = 'solid')
        plt.plot(testacc, label="Test "+str(fold), color=color, linewidth=2.0, linestyle = 'dashed')
        
        plt.subplot(222)
        plt.plot(trainloss, label="Train "+str(fold), color=color, linewidth=2.0, linestyle = 'solid')
        plt.plot(testloss, label="Test "+str(fold), color=color, linewidth=2.0, linestyle = 'dashed')
    ...
    plt.savefig("Grid-kfold2-1layer-%depochs.png" %(epochs), dpi=300)
    # plt.legend(optimizer_list, bbox_to_anchor=(0, 0, 1, 1), bbox_transform=plt.gcf().transFigure)
    # plt.show()
  
if __name__ == '__main__':
    parse = argparse.ArgumentParser('gridplot-kfold2-1layer.py', description='Multi Layer Perceptron classifier.')
